...
def get_pip_binary(binary_name):
    if sys.platform == 'win32':
        venv_exe = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'venv', 'Scripts', f"{binary_name}.exe")
        if os.path.exists(venv_exe):
            return venv_exe
        venv_win_exe = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'venv_win', 'Scripts', f"{binary_name}.exe")
        if os.path.exists(venv_win_exe):
            return venv_win_exe
    else:
...
def clip_youtube_video():
    ...
    try:
        ...
        if ytdlp_bin == 'yt-dlp' or not os.path.isabs(ytdlp_bin):
            ytdlp_base = [sys.executable, "-m", "yt_dlp"]
        else:
            ytdlp_base = [ytdlp_bin]
        ...
        if has_curl_cffi:
            download_cmd.insert(len(ytdlp_base) + 3, "chrome")
            download_cmd.insert(len(ytdlp_base) + 3, "--impersonate")

        result = subprocess.run(download_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=480)
        stderr_text = result.stderr.decode('utf-8', errors='ignore')
        
        # If initial download failed and impersonate was used, retry without impersonate
        if (result.returncode != 0 or not os.path.exists(raw_download_path) or os.path.getsize(raw_download_path) == 0) and has_curl_cffi:
            print("Impersonated download failed, retrying standard download...")
            fallback_cmd = [c for c in download_cmd if c not in ('--impersonate', 'chrome')]
            result = subprocess.run(fallback_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=480)
            stderr_text = result.stderr.decode('utf-8', errors='ignore')
