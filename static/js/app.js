...
function initMergeForm() {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            showToast(`❌ Error: ${err.message || 'Connection failed — refresh page'}`, 'error', 7000);
        } finally {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast(`❌ ${data.error || 'Clip creation failed'}`, 'error', 7000);
            }
        } catch (err) {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast(`❌ ${data.error || 'Generation failed'}`, 'error', 7000);
            }
        } catch (err) {
